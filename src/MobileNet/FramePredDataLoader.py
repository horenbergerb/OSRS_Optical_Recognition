...
class FramePredictionDataset(Dataset):
    def __init__(self, frames_dir, device, in_frames=3, out_dist=6):
    ...
    def __len__(self):
        return self.frames.shape[1]-(self.in_frames+self.out_dist)
    ...
    def __getitem__(self, idx):
        ...
        '''
        sample = {'input': self.frames[:, idx:idx+self.in_frames, :, :],
                  'target': self.targets[:, idx+self.in_frames+self.out_dist, :, :],
                  'label': self.labels[:, idx+self.in_frames+self.out_dist, :, :].flatten(start_dim=1).squeeze()}
